...
#-----------------------------------------------------------------------
    def read_int(self, reg):
        return(0)
        # disable reading for now because it did get 0 Bytes and errored out
        tries = 0
        while(True):
...
#-----------------------------------------------------------------------
    # was write_reg before, but renamed to write_reg_check to mimic the behaviour of the following function
    # which was disabled due to it trying to read the values as well (which doesn't work, see read_int())
...
#-----------------------------------------------------------------------
    # disabled for now because it tries to read the values as well
    def DISABLED__write_reg_check(self, reg, val):
        IFCNT           =   0x02
